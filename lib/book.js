...
var book = function(options, redisOptions){  
  ...
  if(!redisOptions){ redisOptions = {}; }
  if(redisOptions.fake === true || redisOptions.local === true){
    this.redis = require('fakeredis').createClient();
  }else{
    this.redis = ioredis.createClient(redisOptions);
  }
...
book.prototype.addChapter = function(priority, name, options){
  ...
  var safe = true;
  self.chapters.forEach(function(existingChapter){
    if(existingChapter.priority === priority){
      var error = new Error('Already loaded a chapter of priority ' + priority);
      self.logger.log('quitting Empujar due to error', 'emerg');
...
book.prototype.redisListStore = function(collection, key, callback){
  var self = this;
  collection = 'empujar:' + self.options.name + ':' + collection;
  self.redis.rpush(collection, key, callback);
};

book.prototype.redisListPop = function(collection, callback){
  var self = this;
  collection = 'empujar:' + self.options.name + ':' + collection;
  self.redis.lpop(collection, callback);
};

book.prototype.tryForMaster = function(callback){
  // TODO
...
book.prototype.run = function(callback){
  ...
  self.loadChaptersIntoRedis(function(error){
    if(error){ return errorHandler(error); }
    self.workChapters(function(error){
      if(error){ return errorHandler(error); }
...
book.prototype.loadChaptersIntoRedis = function(callback){
  var self = this;
  var jobs = [];
  if(self.chapters.length === 0){ return callback(); }

  self.tryForMaster(function(error, isMaster){
    if(error){ return callback(error); }
    if(!isMaster){ return callback(); }
    var started = 0;
    self.chapters.forEach(function(ch){
      started++;
      if( self.options.chapters.length === 0 || self.options.chapters.indexOf(ch.priority) >= 0 ){
        self.redisListStore('chapters', ch.priority, function(){
          started--;
...
book.prototype.workChapters = function(callback){
  var self = this;
  self.redis.lpop('empujar:chapters', function(error, chapterNumber){
    if(error){ return callback(error); }
    if(!chapterNumber){ return callback(); }

    chapterNumber = parseInt(chapterNumber);
    var ch;
    self.chapters.forEach(function(i){if(i.priority === chapterNumber){ ch = i; }});

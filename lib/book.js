...
book.prototype.ensurePid = function(){
  ...
  if(fs.existsSync(pidFile)){
    oldpid = fs.readFileSync(pidFile);
    if( isrunning(parseInt(oldpid)) ){
      ok = false;
      self.log( 'empujar already running (pid ' + String(oldpid) + ')', 'alert' );
      process.exit();
    }
  }
        ...
        if(connection.connection){
          ...
          connection.connection.connect(function(error){
            if(error){ 
              next(
                new Error(
                  'Cannot connect to: `' + connection.name + '` (' + connection.options.type + '): ' + String(error)
                )
              );
            }
            else{ 

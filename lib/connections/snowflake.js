...
//CODE REVIEW - No variables displayed in snowflake , but keeping it as I see it called at some places ..
        ...
        self.showColumns(table, function(error, columnData) {
          ...
          if(tables.indexOf(table) < 0) {
            self.copyTableSchema(tmp_table, table, function(error, rows) {
              if(error) {
                console.log('Error in copying table Schema. Exiting out')
                callback(error)
              }
              ...
              if(columnDifference.length > 0) {
                ...
                if(mainTableKeys.indexOf(column) < 0) {
                  ...
                  self.addColumnUsingDataType(table, column, columnDefinition, function(error, rows) {
                    if(error) {
                      console.log('Error altering table ' + table + ' for column ' + column)
                    }
                    else {
                      self.insertData(table, data, callback, mergeOnDuplicates, mergeKey)
                    }
                ...
                if(columnList.length > 0) {
                  self.alterTableModifyColumns(table, columnList, function(error){
                    if(error) {
                      console.log('Error while doing alter table for Text columns')
                    } else {
                      self.insertData(table, data, callback, mergeOnDuplicates, mergeKey);
                    }
                  });
                  ...
                  mergeQuery += ')'
                  // console.log('Running following Merge Query -: \n' + mergeQuery)
                  self.query(mergeQuery, function(error, rows){

                    if(error) {
                      console.log('Error While running merge query')
                      callback(error, rows);
                    } else {
                      ...
                      self.query(clearQuery, function (clearError, clearRows) {
                        ...
                        if(clearError) {
                          console.log('Error clearing out temp table')
                        } else {
        ...
        if(missingKey){
          ...
          if(columnSet.size > 0) {
            self.alterTableModifyColumns(table, columnSet, function (error, rows) {
              ...
              if(error) {
                console.log('Error Modifying Columns in createtemporaryTable')
              } else {

  ...
  self.query(query, function(error, rows){
    if(!error && rows && rows.length > 0){
      rows.forEach(function(row){
        rowObj[row.Field] = { type: row.Type };
      });
    }
...
connection.prototype.queryStream = function(query, callback){
  ...
  return self.connection.query(query).stream({highWaterMark: 10});
};
...
connection.prototype.insertData = function(table, data, callback, mergeOnDuplicates){
  ...
  if(!mergeOnDuplicates){ mergeOnDuplicates = true; }
        ...
        if(missingKey){
          ...
          thisData.forEach(function(row){
            ...
            keys.forEach(function(key){
              ...
              if(updatableColumns.indexOf(key) >= 0 && row[key] !== null && row[key] !== undefined){
